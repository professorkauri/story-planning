...
function buildDexEntries(species, versionIdByName, VERSION_TO_GAME_ID){
  const out = {};
  const bestByText = new Map(); // text -> { versionName, versionId, text }

  for(const ft of species.flavor_text_entries || []){
    if(ft.language?.name !== "en") continue;
    const text = (ft.flavor_text || "").replace(/\s+/g," ").trim();
    if(!text) continue;
    const vName = ft.version?.name; if(!vName) continue;
    const vId = versionIdByName.get(vName) ?? Number.MAX_SAFE_INTEGER;

    const cur = bestByText.get(text);
    if(!cur || vId < cur.versionId){
      bestByText.set(text, { versionName: vName, versionId: vId, text });
    }
  }

  for(const { versionName, text } of bestByText.values()){
    const gameId = VERSION_TO_GAME_ID.get(versionName) || versionName;
    out[gameId] = out[gameId] || { regionalDexNumber:"", entry:text };
  }
  return out;
}
